...
#===========================================================
# Check if we can use the Y2Y method to predict
p<- ggplot(filter(mc,year %in% c(2002:target.year),month %in% c(4,7:target.mon)), aes(year, cn))
p1 <- p + geom_line(aes(colour = factor(month)), alpha = 0.6)+geom_point(aes(colour = factor(month)))+
  labs(title="MC History Tourist Number Groupby Month")
print(p1)
...
cmbjp=select(filter(mc,year %in% c(2009:target.year),month %in% c(1:target.mon)),year,month,cn)
...
#2012---2014
(hcor.LR=cor(dmjp[4:6,which(names(dmjp)==gcol[target.mon])],dmjp[4:6,-c(1,which(names(dmjp)==tm))],use="na.or.complete"))
#[ ]3.3 Diff12 LR: M7
...
(hcor.LR=cor(usedata[2:8,which(names(usedata)==gcol[target.mon])],usedata[2:8,-c(1,which(names(usedata)==tm))],use="na.or.complete"))

#ufit=lm(useformula,data=na.omit(usedata))
ufit=lm(useformula,data=filter(na.exclude(usedata), year %in% 2009:2015))
(smfit=summary(ufit))
...
benchmark=data.frame(date=as.Date(target.time), id="1.1",methdology=methodology,target=tm, dependent=um,
                    ...
                    status="Perfect", adopted="Y",stringsAsFactors = F, row.names=NULL)
(benchmark)
...
benchmark[which(gmethod==methodology),]=
          ...
          status="Good", adopted="Y",stringsAsFactors = F, row.names=NULL)
(benchmark)
...
p<- ggplot(filter(mc,year %in% c(2002:target.year),month %in% c(7:target.mon)), aes(year, diff12))
p1 <- p + geom_line(aes(colour = factor(month)), alpha = 0.6)+geom_point(aes(colour = factor(month)))+
  labs(title="MC Y2Y Diff12 Groupby Month")
print(p1)
(hcor.Diff=cor(dmpjp[which(dmpjp$year>=2011),which(names(dmpjp)==gcol[target.mon])],
               dmpjp[which(dmpjp$year>=2011),-c(1, which(names(dmpjp)==tm))],use="na.or.complete"))
...
ufit=lm(useformula,data=filter(na.exclude(usedata), year %in% 2011:2015))
(smfit=summary(ufit))
...
sjp.data=(dmpjp[dmpjp$year>=2011,-1]-dmpjp[dmpjp$year>=2011,which(names(dmpjp)==tm)])
sigma=apply(sjp.data,2,function (x) {return(var(scale(x,scale=F),na.rm=T))})
(sigma=sigma[order(sigma)])
(p <- ggplot(data=melt(as.data.frame(t(sigma)),id=tm),aes(x=factor(variable),weight=value)) + 
  geom_bar(position='dodge',fill="grey")+labs(title=paste(gpicname," : (Diff12 Mx- Diff12 M7) variance",sep=" ")))

p<- ggplot(melt(dmpjp[,c("year","m45","m67","m7","m8")],id="year"), aes(year, value))
p1 <- p + geom_line(aes(colour = factor(variable)), alpha = 0.6)+geom_point(aes(colour = factor(variable)))+
  labs(title="MC Diff12 Number Groupby Year")
print(p1)
...
#===========================================================
# Prepare the data
diff1=select(filter(mc,year %in% c(2009:target.year),month %in% c(1:target.mon)),year,month,diff)
...
benchmark[which(gmethod==methodology),]=
  data.frame(date=as.Date(target.time),id="3.1",methdology=methodology, target=paste(tm,um,sep = "-"), dependent=um,
             ...
             status="Poor", adopted="N",stringsAsFactors = F, row.names=NULL)
(benchmark)
...
diff2nd=select(filter(mc,year %in% c(2009:target.year),month %in% c(1:target.mon)),year,month,diff2nd)
...
hwm=select(fhw.all,date,cn,holiday,newyear,nbmonth,cn2hk,mcjd1,hkmclygl1,hkmcly1,mclydt1,mctq,mcdb1,mcdc1)
...
time.range=c(which(hwm$date==as.Date("2015-02-01")):which(hwm$date==as.Date("2015-07-01"))) #terrible
cat("The Date We Use in this algorithm\n", as.character(hwm[time.range,"date"]),"\n")


hwm.p=hwm[which(hwm$date==as.Date("2015-08-01")),]
...
tmc=filter(tmc, date>=as.Date("2014-08-01") & date<=as.Date("2015-08-01"))
tmc
